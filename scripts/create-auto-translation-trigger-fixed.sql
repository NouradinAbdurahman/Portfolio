...
BEGIN
    ...
    webhook_url := COALESCE(
        ...
        'http://localhost:3001/api/translate/webhook'
    );
    ...
    ELSIF TG_TABLE_NAME = 'projects' THEN
        -- Handle project updates (only if projects table exists)
        translation_data := jsonb_build_object(
    ...
    ELSIF TG_TABLE_NAME = 'services' THEN
        -- Handle service updates (only if services table exists)
        translation_data := jsonb_build_object(
...
-- Create trigger for translations table (this should exist)
...
-- Only create triggers for projects table if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'projects') THEN
        DROP TRIGGER IF EXISTS project_translation_trigger ON projects;
        EXECUTE 'CREATE TRIGGER project_translation_trigger
            AFTER INSERT OR UPDATE ON projects
            FOR EACH ROW
            EXECUTE FUNCTION handle_content_translation()';
        RAISE NOTICE 'Created trigger for projects table';
    ELSE
        RAISE NOTICE 'Projects table does not exist, skipping trigger creation';
    END IF;
END $$;

-- Only create triggers for services table if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'services') THEN
        DROP TRIGGER IF EXISTS service_translation_trigger ON services;
        EXECUTE 'CREATE TRIGGER service_translation_trigger
            AFTER INSERT OR UPDATE ON services
            FOR EACH ROW
            EXECUTE FUNCTION handle_content_translation()';
        RAISE NOTICE 'Created trigger for services table';
    ELSE
        RAISE NOTICE 'Services table does not exist, skipping trigger creation';
    END IF;
END $$;
